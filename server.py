...
class Channel:
    def __init__(self,name,server):
        ...
        self.is_invisible = lambda : self.name[1] == '.'
    ...
    def user_quit(self,user,reason='quitting'):
        ...
        user.event(self,'part',self.name)
        for u in self.users:
            if not self.is_anon():
                u.event('%s!anon@nameless'%user.nick,'part',self.name)
            else:
                u.send_notice(self,'%s -- %s online'%(self.name,len(self.users)))  
        if self.empty():
    ...
    def privmsg(self,orig,msg):
        for user in self.users:
            if user == orig:
                continue
            src = 'anonymous!anonymous@%s'%self.server.name
            if not self.is_anon():
                src = '%s!anon@nameless'%orig.nick
                if user == orig:
                    src = orig.user_mask()
